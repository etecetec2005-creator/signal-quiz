...
st.markdown("""
    ...
    /* 2. 選択ボックス自体の高さを自動調整（文字が重ならないようにする） */
...
CATEGORIES = [
    ...
    "標識類（灯具あり）（入換標識反応灯）", "標識類（灯具あり）（線路表示器）", "標識類（灯具あり）（出発反応標識）",
    "標識類（灯具あり）（列車停止標識）", "標識類（灯具あり）（車両停止標識）", "標識類（灯具あり）（車止標識）",
    "標識類（灯具なし）（列車停止標識）", "標識類（灯具なし）（車両停止標識）", "標識類（灯具なし）（車止標識）",
    "諸標類（灯具あり）（番線表示灯）", "諸標類（灯具あり）（限界表示灯）", "諸標類（灯具あり）（線路別表示灯）",
    "諸標類（灯具あり）（乗務員呼出表示灯）", "諸標類（灯具あり）（移動・機器取扱い禁止表示器）",
    "諸標類（灯具なし）（番線表示標）", "諸標類（灯具なし）（信号鳴呼位置標）", "諸標類（灯具なし）（停止限界標識）",
    "電気転てつ機（状態監視有り）", "電気転てつ機（状態監視無し）", "電気転てつ機（YS形）",
    "発条転てつ機", "転てつ転換鎖錠装置", "回路制御器", "鉄管装置", "継電連動装置", "電子連動装置",
    "列車集中制御装置", "自動進路制御装置", "列車運行状況表示装置", "ATS装置", "分岐器速度制限警報装置",
    "踏切警報機", "踏切遮断機", "踏切制御子", "踏切機器類", "踏切支障報知装置", "表示装置", "電子踏切制御器",
    "踏切電源機器", "踏切器具箱類", "直流軌道回路", "パルス軌道回路", "AFO軌道回路", "商用軌道回路",
    "信号ケーブル", "支持物", "管路", "信号器具箱類", "限界支障報知装置", "落石警報裝置",
    "安全側線緊急防護裝置", "支障報知裝置", "横取裝置用停止現示裝置", "トンネル内支障報知裝置",
    "落下物検知裝置", "ホーム用非常ボタン装置", "接近警報装置", "旅客通路用接近表示装置",
    "固定式列車接近警報装置", "諸設備踏切遮断機", "諸設備踏切制御子", "諸設備機器類",
    "諸設備電子踏切制御器", "諸設備踏切支障報知裝置", "諸設備電源機器", "諸設備器具箱類",
    "抑止表示装置", "消防用設備", "ブレーキ支援装置", "発動発電機", "配電盤", "整流器",
    "自動電圧調整器", "蓄電池", "分倍周軌道回路", "100Hz 軌道回路", "分周軌道回路",
    "機器集中形AF軌道回路", "無絶縁軌道回路", "長大軌道回路", "H・DC軌道回路", "H・AC軌道回路",
    "列車検知装置", "80Hzコード軌道回路"
]
...
def fetch_quizzes(category):
    ...
    try:
        ...
        st.error(f"エラーが発生しました。再試行してください。")
        return []
